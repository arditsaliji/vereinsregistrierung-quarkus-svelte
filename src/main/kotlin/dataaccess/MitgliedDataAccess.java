...
public class MitgliedDataAccess {
    ...
    public List<Mitglied> getFilteredMitglieder(String filteredString) throws Exception {
        List<Mitglied> mitglieder = new ArrayList<>();
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
        } catch (ClassNotFoundException e) {
        ...
        try (Connection connection = DriverManager
            ...
            return mitglieder.stream().filter(mitglied -> mitglied.getStringForFilter().contains(filteredString)).collect(Collectors.toList());
        } catch (Exception e) {
            throw new Exception(e.getMessage());
        }
    }
